...
export class ExternalTvService {
  ...
  constructor(
    ...
    private readonly wall?: (action: 'open' | 'close') => Promise<LauncherResult>,
  ) {}
  get() {
    return { ...this.status };
  }
  ...
  async open(rawUrl: string) {
    ...
    try {
      ...
      const opened = await this.run('open', source.url);
      if (!opened.Ok) {
  ...
  private async run(action: 'open' | 'close' | 'status', url?: string): Promise<LauncherResult> {
    if (this.runner) return this.runner(action, url);
  ...
  private async runWall(action: 'open' | 'close') {
    if (this.wall) return this.wall(action);
    if (process.env.LIVEWALL_TEST_WALL_CONTROL === '1')
      return {
        Ok: true,
